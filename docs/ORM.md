...
**Not yet available (planned):** [Production HttpStore](ROADMAP.md#production-httpstore) phase **0.12** (GSP `sync_mirror`), multi-valued and language-tagged fields (**0.13**). HTTP resilience (retries, batched UPDATE, SELECT GET/POST) shipped in **0.11.0**. See [SPARQLMojo parity backlog](ROADMAP.md#sparqlmojo-parity-backlog). **Mirror semantics** (replace-on-pull, `mirror_mode`) shipped in **0.10.0**. **Session cache control** (`merge`, `refresh`, `expunge`, `expunge_all`) shipped in **0.9.0**. **HttpStore** partial mirror pull and hydration fixes shipped in **0.9.1**; **`refresh` mirror pull**, **`merge` partial-field fix**, and **IRI ref hydration** in **0.9.2**. **Query lists** (`offset`, `order_by`, `count`, nullable `OPTIONAL`) shipped in **0.8.0**. **Async session and stores** shipped in **0.6.0**.
