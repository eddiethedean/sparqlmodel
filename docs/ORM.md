...
This guide is for **application developers**. SparqlModel is the ORM; **[TripleModel](https://github.com/eddiethedean/triplemodel)** (`triplemodel>=0.12.0` for SparqlModel **0.13+**; `>=0.10.0` for older releases), installed automatically, is the mapping engine underneath. In-process graphs use **pyoxigraph** via `triplemodel.Store`.
...
| **Dependency** | Requires `triplemodel>=0.12.0,<2` (0.13+) and `pyoxigraph` | Standalone library |
